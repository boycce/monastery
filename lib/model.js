...
Model.prototype._setupFieldsAndWhitelists = function(fields, path) {
  /**
   * Setup fields and whitelists / blacklists. This can be called mulitple times.
   * @param {object} fields - can be a nested subdocument or array
   * @param {string} <path>
   */
  this._setupFields(fields)
  this.findWL = this._getWhitelist(fields, path || '')
...
Model.prototype._setupIndexes = function(fields) {
  ...
  return (model.manager._state == 'open'? Promise.resolve() : model.manager).then(data => {
    let collection = model.manager._db.collection(model.name)
    ...
    .then(indexes1 => {
      ...
      // console.log(1, indexes1)
      for (let index1 of indexes1) {
        if (!index1.textIndexVersion) continue
        for (let index2 of indexes) {
          let fieldsInTextIndex1 = Object.keys(index1.weights).sort().join()
          let fieldsInTextIndex2 = Object.keys(index2.key).sort().join()
          if (fieldsInTextIndex1 !== fieldsInTextIndex2) {
            model.info(`Text index options are different for '${index1.name}', removing old text index`)
            indexNames.push(index1.name)
          }
        }
      }
