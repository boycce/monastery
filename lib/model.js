...
let Model = module.exports = function(name, opts, manager) {
  ...
  Object.assign(this, {
    ...
    insertBL: opts.insertBL || ['createdAt', 'updatedAt'],
    updateBL: opts.updateBL || ['createdAt', 'updatedAt'],
  ...
  for (let key in (this._collection || {})) {
    if (key.match(/^manager$|^options$|^_|^middlewares$|^name$/)) return
    this['_' + key] = this._collection[key]
  }
...
Model.prototype._setupIndexes = function(fields) {
  ...
  } else if (!model.manager.connected()) {
    let error = `Skipping createIndex on the '${model.name}' model, no mongodb connection found.`
    return Promise.reject(error)
  }

  // Create indexes
  return model.manager.then(data => {
    return data.collection(model.name).createIndexes(indexes).then(data => {
      model.debug('db index created:', data)
    })
  })
