...
module.exports = {
  ...
  validate: function(data, opts, cb) {
     ...
     *     @param {array|string} skipValidation - skipValidation on these fields
      ...
      util.forEach(fields, function(field, fieldName) {
        ...
        if (util.isDefined(schema.default)) {
          if (schema.defaultOverride || (opts.insert && !util.isDefined(value))) {
            if (fieldName != 'insertedAt' || timestamps) {
              value = util.isFunction(schema.default)? schema.default(this.manager) : schema.default
            }
          }
          // UpdatedAt is the only default field always being updated
          if (fieldName == 'updatedAt' && timestamps && !util.isDefined(value)) {
            value = util.isFunction(schema.default)? schema.default(this.manager) : schema.default
          }
        }
        ...
        // Fields can be a subdocument
          ...
          if (util.isObject(value) || opts.insert || (path2||'').match(/\./)) {
  ...
  _validateRules: function(field, value, opts, path) {
    ...
    for (let ruleName in field) {
      ...
      // ignore root undefined properties for update
      if (util.isUndefined(value) && opts.update && !path.match(/\./)) continue
  ...
  _ignoredRules: [
    ...
    'default', 'defaultOverride', 'fileSize', 'formats', 'image', 'index', 'insertOnly', 'model', 'nullObject', 'type', 'virtual'
  ]
