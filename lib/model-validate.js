...
module.exports = {
  ...
  _validateRules: function(field, value, opts, path) {
    ...
    if (opts.skipValidation.length) {
      ...
      for (let skipRule of opts.skipValidation) {
        // Make sure there is numerical character representing arrays
        let skipRuleChunks = skipRule.split('.')
        for (let i=0, l=pathChunks.length; i<l; i++) {
          if (pathChunks[i].match(/^[0-9]+$/) && !skipRuleChunks[i].match(/^\$$|^[0-9]+$/)) {
            skipRuleChunks.splice(i, 0, '$')
          }
        }
        for (let i=0, l=skipRuleChunks.length; i<l; i++) {
          if (skipRuleChunks[i] == '$') skipRuleChunks[i] = '[0-9]+'
        }
        if (path.match(new RegExp('^' + skipRuleChunks.join('.') + '(\.|$)'))) return []
      }
    }
