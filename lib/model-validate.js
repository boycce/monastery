...
Model.prototype._validateRule = function (dataRoot, ruleName, fieldSchema, ruleArg, value, opts, path) {
  ...
  // Ignore null (if nullObject is set on objects or arrays) (nullObject=true this should be default)
  if (value === null && !rule.validateNull && (fieldSchema.isObject || fieldSchema.isArray) && fieldSchema.nullObject) return

  // Ignore null
  if (value === null && !rule.validateNull && !(fieldSchema.isObject || fieldSchema.isArray)) return
  ...
  if (!rule.fn.call(dataRoot, value, ruleArg, path, this)) {
    let ruleMessageKey = this.messagesLen && util.getMostSpecificKeyMatchingPath(this.messages, path)
...
Model.prototype._ignoredRules = [ 
  ...
  'image', 'index', 'insertOnly', 'model', 'nullObject', 'optionalEmbed', 'params', 'path', 'getSignedUrl', 'timestampField',
