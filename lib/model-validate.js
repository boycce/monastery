...
module.exports = {
  ...
  validate: function(data, opts, cb) {
    ...
    return util.runSeries(this.beforeValidate.map(f => f.bind(opts, data))).then(() => {
      return this._validateFields(this.fields, data, opts, '')

    // Success/error
    }).then(data => {
      if (data[0].length) throw data[0]
      if (cb) cb(null, data[1])
      else return Promise.resolve(data[1])
  ...
  _validateFields: function(fields, data, opts, path) {
    ...
    util.forEach(util.forceArray(data), function(data, i) {
      util.forEach(fields, function(field, fieldName) {
        ...
        // Ignore blacklisted
        if (opts.blacklist.indexOf(fieldName) >= 0 && !schema.defaultOverride) return
        // Ignore insert only
