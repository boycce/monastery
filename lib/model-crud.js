...
module.exports = {
  ...
  insert: function(opts, cb) {
    ...
    let options = util.omit(opts, ['data', 'insert', 'model', 'respond', 'whitelist'])
  ...
  find: function(opts, cb) {
    ...
    options.limit = opts.one? 1 : parseInt(options.limit || 25)
    options.projection = options.project || { ...this.findWLProject }
    ...
    if (opts.whitelist) {
      ...
      for (let key in whitelist) {
        ...
        for (let i=split.length; i--;) {
          ...
          if (options.projection[split.join()]) delete options.projection[key]
        }
      }
    }
  ...
  update: function(opts, cb) {
    ...
    // Operation options
    let options = util.omit(opts, ['data', 'query', 'respond', 'whitelist'])
    options.sort = options.sort || { "createdAt": -1 }
    options.limit = parseInt(options.limit || 25)
