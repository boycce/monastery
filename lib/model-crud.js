...
module.exports = {
  ...
  insert: function(opts, cb) {
    ...
    return this.validate(data, { ...opts }).then(data => {
      if (!data || util.isEmpty(data)) throw 'No valid data passed in to insert..'
  ...
  find: function(opts, cb) {
    ...
    opts = opts || {}
    let lookups = []
    let query = util.isObjectAndNotID(opts.query)? util.removeUndefined(opts.query) : { _id: opts.query }
  ...
  update: function(opts, cb) {
    ...
    return this.validate(data, { ...opts }).then(data => {
      if (!data || util.isEmpty(data)) throw 'No valid data passed in to update..'
  ...
  _removeBlacklisted: function(data, whitelist) {
    /**
     * Remove blacklisted fields, and recurses through fields that are models.
     * @param {object|array} data
    ...
    for (let item of modeldata) {
      for (let key in item.dataRef) {
        let fieldnamePath = item.fieldname? item.fieldname + '.' + key : key
        if (model[item.modelName].findBL.includes(key) && !whitelist.includes(fieldnamePath)) {
          delete item.dataRef[key]
        }
      }
    }
    return data
