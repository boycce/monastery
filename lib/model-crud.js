...
module.exports = {
  ...
  update: function(opts, cb) {
    ...
    return Promise.resolve().then(() => {
      ...
      if (!data || util.isEmpty(data)) throw new Error(`No valid data passed to ${this.name}.update()`)
  ...
  remove: function(opts, cb) {
    ...
    return Promise.resolve().then(() => {
      ...
      return util.runSeries(this.beforeRemove.map(f => f.bind(opts, null)))
  ...
  _queryObject: function(opts) {
    ...
    if (!util.isId(opts.query._id) && util.isString(opts.query._id)) {
      throw new Error('Please pass a valid MongoId to options.query')
    }
