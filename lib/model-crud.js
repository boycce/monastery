...
module.exports = {
  ...
  find: function(opts, cb) {
    ...
    // Format query
    if (util.isId(opts.query) || util.isString(opts.query)) opts.query = { _id: opts.query }
    else opts.query = util.removeUndefined(opts.query)
    if (util.isId(opts.query._id) || util.isString(opts.query._id)) opts.one = true
    if (util.isId(opts.query._id)) opts.query._id = this.manager.id(opts.query._id) // for aggregation
