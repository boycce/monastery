...
module.exports = {
  ...
  insert: function(opts, cb) {
    ...
    opts = opts || {}
    opts.insert = true
    let data = util.parseDotNotation(opts.data || req.body)
    ...
    return util.runSeries(this.beforeValidate.map(f => f.bind(operation, data))).then(() => {
      ...
      return this._insert(this.name, data)
  ...
  find: function(opts, cb) {
    ...
    if (opts.populate) {
      ...
      var operation = this._aggregate.bind(this, this.name, [
        { $match: query },
        { $sort: options.sort },
        { $skip: options.skip },
        { $limit: options.limit },
    ...
    // Normal operation
      ...
      operation = this[opts.one? 'findOne ' : 'find '].bind(null, query, options)
    }
  ...
  findOne: function(opts, cb) {
    return this.find({ ...opts, req: req, one: true }, cb)
  },
  ...
  remove: function(opts, cb) {
    ...
    util.runSeries(this.beforeRemove.map(f => f.bind(operation, null))).then(() => {
  ...
  _callAfterFind: function(data, req) {
    /**
  ...
  _recurseAndFindModels: function(fields, dataArr) {
    ...
    for (let data of util.toArray(dataArr)) {
      util.each(fields, (field, fieldName) => {
        if (!data) return
