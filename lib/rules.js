...
module.exports.defaultRules = {
  ...
  required: {
    ...
    fn: function(x, typeval) {
      if (this.isArray(x) && !x.length) return false
  ...
  'isInteger': {
    ...
    tryParse: function(x) {
      if (this.isString(x) && x.match(/^[\+-]?[0-9]+$/)) return x // keep string nums intact 
      return isNaN(parseInt(x))? x : parseInt(x)
    }, 
    fn: function(x) {
      if (this.isString(x) && x.match(/^[\+-]?[0-9]+$/)) return true 
  ...
  'isId': {
    ...
    tryParse: function(x) {
      // Try and parse value to a mongodb id object, which needs this.db to be defined
      if (x === '') return null
      let checkForHexRegExp = RegExp("^[0-9a-fA-F]{24}$")
      if (this.isString(x) && x.match(checkForHexRegExp) && this.db) return this.db.id(x)
      else return x
    },
    fn: function(x) {
      ...
      return this.isObject(x) && x.toString().match(checkForHexRegExp)? true : false
    }
  },
  'max': {
  ...
  'regex': {
    ...
    fn: function(x, regex) {
      if (this.isRegex(regex)) return validator.matches(x, regex)
