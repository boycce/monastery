...
module.exports = {
  ...
  formatDuplicateKeyError: function (model, err, nullIfNoCustomMessage) {
    /**
     * Formats a MongoDB duplicate key error into a validation error using the model's custom messages
     * @param {object|undefined} model - the model that the error occurred in
     * @param {Error} err - the error object
     * @param {boolean} nullIfNoCustomMessage - whether to return null if no custom message is found
     * @return {object|null} the formatted error object
     * 
     * Example duplicate key error from MongoDB: 
     * {
     *   code: 11000,
     *   keyPattern: { email: 1, uid: 1 },
     *   keyValue: { email: 'hi@domain.com', uid: '70' },
     *   message: 
     *     'E11000 duplicate key error collection: myproject.document index: settings.number_1 ' +
     *     'dup key: { company: ObjectId('68c266...'), settings.number: "Q10030" }'
     * }
     */
    if (!err || err.code !== 11000 || Array.isArray(err)) return null
    let detail = err.message
  
    // Mongo does not expose index name on the error object; parse it from the message.
    const keys = Object.keys(err.keyPattern || {})
    const indexMatch = detail.match(/index:\s+(\S+)\s+dup key/i)
    let indexName = indexMatch?.[1]?.replace(/(?:_-?[0-9]+|_)$/, '') // removes trailing _1 or _-1 from the index name
    let modelName = model?.name || detail.match(/collection:\s+\S+\.(\S+)/)?.[1]
    if (!indexName) indexName = keys.join('.')

    // Get the message from the model's custom messages
    if (model?.messagesLen && indexName) {
      const ruleMessage = model.messages[indexName]?.index_unique
      const value = keys.length === 1 && err.keyValue ? err.keyValue[indexName] : err.keyValue
      if (ruleMessage) {
        detail = module.exports.isFunction(ruleMessage)
          ? ruleMessage.call(null, value, undefined, indexName, model)
          : ruleMessage
      } else if (nullIfNoCustomMessage) {
        return null
      }
    } else if (nullIfNoCustomMessage) {
      return null
    ...
    return [{
      status: '400',
      title: indexName || 'error',
      detail: detail,
      meta: { 
        rule: 'index_unique', 
        ...(modelName ? { model: modelName } : {}),
        ...(indexName ? { field: indexName.match(/[^.]+$/)[0] } : {}),
      },
    }]
  },

  getMostSpecificKeyMatchingPath: function (object, path) {
    /**
